              ...
              <div>
                ...
                <div className="relative">
                  <input
                    type="text"
                    inputMode="numeric"
                    value={filters.umkreis === 0 ? '' : filters.umkreis}
                    onChange={e => {
                      const val = e.target.value.replace(/\D/g, '');
                      setFilters({ ...filters, umkreis: val === '' ? 0 : parseInt(val) });
                    }}
                    className="w-full px-4 py-2 border rounded-lg pr-12"
                    placeholder="z.B. 30"
                  />
                  <span className="absolute right-4 top-2 text-gray-500">km</span>
                </div>
                <p className="text-xs text-gray-500 mt-1">Leer lassen = Ganz Deutschland</p>
              </div>
